...
def create_summary_table(metrics_bundle: Dict[str, object]) -> pd.DataFrame:
    ...
    row = {
        ...
        "Max Floating Drawdown %": metrics_bundle.get("max_floating_drawdown_percent"),
        "Peak-to-Trough Equity Drawdown %": metrics_bundle.get("peak_equity_drawdown_percent"),
...
def create_pair_table(summary: Dict[str, object]) -> pd.DataFrame:
    ...
    for pair, info in sorted((summary.get("pairs") or {}).items()):
        rows.append({
            ...
            "Take Profit": info.get("take_profit", info.get("baseline_take_profit")),
            "Grid": info.get("grid_size", info.get("baseline_grid_size")),
            "Max Trades": info.get("max_trades", info.get("baseline_max_trades")),
